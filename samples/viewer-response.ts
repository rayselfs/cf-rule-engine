 ...
 *
 * Example CloudFront Function viewer-response configuration.
...
export default defineViewerResponse([
  ...
  // For requests to stream-stage.example.com with origins from *.example.com,
  // echo back the origin (ACAO = Access-Control-Allow-Origin).
  {
    criteria: all([
      hostnameIs(['stream-stage.example.com']),
      (req) => (req.headers['origin']?.value ?? '').includes('.example.com'),
    ]),
